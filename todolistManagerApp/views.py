...
def task_list(request):
    ...
    if is_admin_user:
        ...
        user_task_count = tasks.count()  # Admin can update all tasks
    else:
        tasks = Task.objects.filter(assigned_users=request.user)  # Regular users see only their tasks
        user_task_count = tasks.count()  # Count tasks assigned to the current user
    ...
    return render(request, 'task_list.html', {
        ...
        'today': today,
        'user_task_count': user_task_count,  # Pass the count to the template
    })
...
def update_task_status(request):
    if request.method == 'POST':
        # Fetch all tasks for admin users, and only user-assigned tasks for regular users
        tasks = Task.objects.all() if request.user.groups.filter(name='Admin').exists() else Task.objects.filter(assigned_users=request.user)
...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.error(request, mark_safe('<span style="font-weight: bold; font-size: 1.3em;">There was an error with your registration. Please check the form.</span>'))
    else:
